import numpy as np
import cv2 as cv
from matplotlib import pyplot as plt

kernel = np.ones((3, 5), np.uint8)


# 이미지를 가져온다.
source = cv.imread('Image/LV3.png', cv.IMREAD_COLOR)
print("source shape:" ,source.shape)
if source is None:
    print("이미지를 불러올 수 없습니다.")
    exit()


#roi 설정

#소스 이미지를 선택한 후 엣지 출력
source_blue, source_green, source_red = cv.split(source)

v = np.median(source_red)
# print(v)

# lower, upper threshold 자동 계산
lower = int(max(0,  0.90*v))
upper = int(min(255, 1.8 * v))

#소스이미지에서 엣지 검출
source_edges = cv.Canny(source_red, lower, upper, apertureSize=3)

#========================================roi===================================

#roi를 만들기 위한 요소 선정
binary = source_edges.copy()
mask= np.zeros_like(binary)

#roi좌표-원본
pts = np.array([[
    (3, 468),  # 왼쪽 위
    (894, 380),  # 오른쪽 위
    (520, 1040),  # 오른쪽 아래
    (3, 1057)    # 왼쪽 아래
]], dtype=np.int32)


#선택한 좌표 내부만 흰색인 마스크 완성
cv.fillPoly(mask,pts,255)

#검출된 엣지와 선정한 부분만 흰색인 부분을 곱하여 roi설정
roi = cv.bitwise_and(mask,source_edges)
roi = cv.GaussianBlur(roi, (5,5), 5)
roi = cv.morphologyEx(roi, cv.MORPH_CLOSE, kernel)

#=======================컨투어===================================================================================

#컨투어 이미지를 넣기 위한 빈 행렬
binary = source_edges.copy()
no_filtered_contours_image = np.zeros_like(binary)



#초기 roi이미지에 대해서 컨투어 찾기
contours_roi, _ = cv.findContours(roi, mode=cv.RETR_EXTERNAL, method=cv.CHAIN_APPROX_SIMPLE)
cv.drawContours(no_filtered_contours_image, contours_roi, -1, 255, 2)


#필터링된 컨투어를 담기위한 배열
filtered_contours_image= np.zeros_like(binary)
contours_filtered = []


for cnt in contours_roi:
 
    x, y, w, h = cv.boundingRect(cnt)
    if w==0: continue
    ratio=h/w
    print(f"ratio: {ratio:.2f}")


area_threshold=1300

for cnt in contours_roi:
    area = cv.contourArea(cnt)
    x, y, w, h = cv.boundingRect(cnt)
    
    if w==0:
        continue
    ratio=h/w
    # if (area >= 1300)  or (600<=area<1300 and ratio>0.7):
    if (area >= 1300):
        contours_filtered.append(cnt)


if not contours_filtered:
    print("필터링된 컨투어가 없습니다.")
    exit()

cv.drawContours(filtered_contours_image, contours_filtered, -1, 255, 2)




# 1. 컨투어 점들 모두 모으기
all_points = np.vstack(contours_filtered).squeeze()

# 2. x 기준 정렬
all_points = all_points[np.argsort(all_points[:, 0])]
x = all_points[:, 0]
y = all_points[:, 1]

# 3. polyfit으로 2차 곡선 근사
coeffs = np.polyfit(x, y, deg=2)
poly = np.poly1d(coeffs)

x_fit = np.linspace(x.min(), x.max(), 100)
y_fit = poly(x_fit)

# 4. 원본 이미지에 곡선 그리기
roi_y_start = filtered_contours_image[:1].max()
source_poly = source.copy()
source_poly = cv.cvtColor(source_poly, cv.COLOR_RGB2BGR)

for i in range(len(x_fit)-1):
    pt1 = (int(x_fit[i]), int(y_fit[i]))
    pt2 = (int(x_fit[i+1]), int(y_fit[i+1]))
    if 0 <= pt1[1] < source.shape[0] and 0 <= pt2[1] < source.shape[0]:
        cv.line(source_poly, pt1, pt2, (0, 255, 0), 2)  




dotted_lines=np.zeros_like(binary)
dotted_lines=cv.cvtColor(dotted_lines, cv.COLOR_GRAY2RGB)

threshold_1 = int(1080-250)
threshold_2 = int(1080-120)

for y_val, color in zip([threshold_1, threshold_2], [(0, 255, 0), (0, 255, 255)]):
    for x_pos in range(0, 1920, 30):
        cv.line(dotted_lines, (x_pos, y_val), (x_pos + 10, y_val), color, 2)



plt.figure()
plt.imshow(source_poly)
plt.title('final')
plt.axis('off')

plt.figure()
plt.imshow(filtered_contours_image,cmap='gray')
plt.title('filtered_contours_image')
plt.axis('off')


plt.figure()
plt.imshow(no_filtered_contours_image,cmap='gray')
plt.title('no_filtered_contours_image')
plt.axis('off')



plt.figure()
plt.imshow(dotted_lines)
plt.title('dotted_lines')
plt.axis('off')
plt.show()